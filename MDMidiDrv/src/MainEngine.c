	...
	switch(ChnMode) {
		case 0x00: // FM
			...
			if((TempChn->UseMode & 0x30) == 0x10)
				NewVol = GetFMVolume(
					GetMidiNoteVolume(TempChn->NoteVolume, 0x09),
					MidChn[0x09].ChnVolB,
					TempChn->UseMode & 0x02
				);
			else
				NewVol = GetFMVolume(
					GetMidiNoteVolume(TempChn->NoteVolume, MIDIChn),
					TempMid->ChnVolB,
					TempChn->UseMode & 0x02
				);
...
static UINT16 GetMappedMidiIns(MIDI_CHN *TempMid) {
	...
	if((TempMid->BnkSel[0x00] & 0x7E) == 0x58) {
		...
		return GetMappedIns(
			&GYBData.InsMap[GYBBANK_DRUM].Ins[TempMid->Ins],
			TempMid->BnkSel[0x01],
			0xFF
		);
	}
	...
	return GetMappedIns(
		&GYBData.InsMap[GYBBANK_MELODY].Ins[TempMid->Ins],
		TempMid->BnkSel[0x00],
		TempMid->BnkSel[0x01]
	);
}
