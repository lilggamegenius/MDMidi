...
static void InterpretMIDI(UINT32 *SampleCount) {
	...
	if(!*SampleCount) {
		...
		while(!FileEnd && VGMPos < VGMDataLen) {
			...
			switch(Command & 0xF0) {
				case 0xF0: switch(Command) {
						case 0xF0: DataLen = GetMIDIDelay(&TempLng);
							VGMPos += TempLng + DataLen;
							break;
						case 0xFF: LoopBack = false;
							switch(VGMData[VGMPos]) {
								case 0x2F: // End Of File
									FileEnd = true;
									break;
								case 0x51: TempLng = VGMSmplPos - TempTempo->BaseTick;
								//if (! TempLng)
								//	TempoCount --;
				...
				case 0xA0:
				case 0xB0:
				case 0xE0: VGMPos += 0x02;
					break;
				case 0xC0:
				case 0xD0: VGMPos += 0x01;
					break;
			}
			if(Command < 0xF0)
				LastMidCmd = Command;
