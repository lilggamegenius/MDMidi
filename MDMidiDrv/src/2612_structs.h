...
typedef struct gyb_instrument_v2{
	char* Name;
	UINT8 Reg[0x20];
} GYB_INSTRUMENT_V2;

typedef struct gyb_instrument_bank_v2{
	UINT8 InsCount;
	UINT8 InsMap[0x80];
	GYB_INSTRUMENT_V2* InsData;
} GYB_INS_BANK_V2;

typedef struct gyb_file_v2{
	UINT8 FileVer;
	UINT8 LFOVal;
...
typedef struct gyb_instrument_chord_v3{
	UINT8 NoteAlloc;
	UINT8 NoteCount;
	INT8* Notes;
} GYB_INS_CHORD_V3;

typedef struct gyb_instrument_v3{
	UINT16 InsSize;
	UINT8 Reg[0x1E];
	UINT8 Transp;
	UINT8 AddData;
...
typedef struct gyb_map_sub_item_v3{
	UINT8 BankMSB;
	UINT8 BankLSB;
	UINT16 FMIns;
} GYB_MAP_ITEM_V3;

typedef struct gyb_map_item_list_v3{
	UINT16 EntryAlloc;
	UINT16 EntryCount;
	GYB_MAP_ITEM_V3* Entry;
} GYB_MAP_ILIST_V3;

typedef struct gyb_map_v3{
	GYB_MAP_ILIST_V3 Ins[0x80];
} GYB_MAP_V3;

typedef struct gyb_instrument_bank_v3{
	UINT16 InsAlloc;
	UINT16 InsCount;
	GYB_INSTRUMENT_V3* InsData;
} GYB_INS_BANK_V3;

typedef struct gyb_file_v3{
	UINT8 FileVer;
	UINT8 LFOVal;
