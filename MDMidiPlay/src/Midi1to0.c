...
static unsigned char CopyMIDIEvent(
	unsigned char *SrcData,
...
unsigned char MIDI1to0(
	...
	unsigned long int DataLen = LittleBigEndianCnvL(TempLng);

	unsigned long int DstLen = SrcLen * 2;
	unsigned char *DstData   = (unsigned char*) malloc(DstLen);
	...
	// Write MIDI Format 0
	unsigned short int TempSht = LittleBigEndianCnvS(0x0000);
	...
	MIDITRK_INF *TrkData = malloc(TrkCnt * sizeof(MIDITRK_INF));
	for(unsigned short int CurTrk = 0x00; CurTrk < TrkCnt; CurTrk++) {
		memcpy(&TempLng, &SrcData[CurPos + 0x00], 0x04);
		if(TempLng != FCC_MTRK)
			return 0xE0;
	...
	do {
		...
		for(unsigned short int CurTrk = 0x00; CurTrk < TrkCnt; CurTrk++) {
			...
			while(CurTData->CurPos < CurTData->TrkEnd) {
				...
				switch(TempByt) {
					case 0x00: EvtsWritten++;
						WriteDelay = true;
						break;
					case 0x01: // Event ignored
						break;
					case 0xFF: // Error
					default: printf("Invalid Event at Pos %lX\n", CurTData->CurPos);
						return 0xFF;
				}
			}
			if(CurTData->CurPos >= CurTData->TrkEnd)
